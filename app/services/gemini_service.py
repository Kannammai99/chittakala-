...
from app.agents.prohibited_language_validator import ProhibitedLanguageValidator, RESPONSIBLE_AI_DISCLAIMER
...
class GeminiReflectionResponse(BaseModel):
    ...
    sanitized: bool = Field(
        False,
        description="True if output was modified by deterministic Responsible AI prohibited language validator"
    )
...
class GeminiReflectionService:
    ...
    def generate_fallback_reflection(self, art_form_title: str = "Indian Folk Art") -> GeminiReflectionResponse:
        """Safe local fallback reflection generator using ADK Multi-Agent Coordinator & ProhibitedLanguageValidator."""
        data = MultiAgentCoordinator.generate_fallback_synthesis(art_form_title)
        sanitized_data, was_mod = ProhibitedLanguageValidator.validate_and_sanitize_response(data)
        return GeminiReflectionResponse(**sanitized_data)
            ...
            try:
                ...
                if response and response.text:
                    ...
                    if "```" in text:
                        ...
                        text = "\n".join(lines).strip()
                    parsed_obj = GeminiReflectionResponse.model_validate_json(text)
                    raw_dict = parsed_obj.model_dump()
                    sanitized_dict, _ = ProhibitedLanguageValidator.validate_and_sanitize_response(raw_dict)
                    reflection = GeminiReflectionResponse(**sanitized_dict)
